/*
	Also called the assigning-investor component/page
*/
...
export default function PublishOffer({ children, ...props }) {
  ...
  const currentOffer =
    currentOfferObj !== null ? JSON.parse(currentOfferObj) : null;
  ...
  useEffect(
    function getCategoriesID() {
      ...
      if (componentIsMounted) setCategoriesIds((state) => [...IDs]);
  ...
  useEffect(
    function multiInvestorsRequest() {
  ...
  const assignInvestors = async (loanOfferId) => {
    ...
    if (investorsValue === null) {
      ...
      setFeedBack((state) => ({
        ...state,
        statusNotSet:
          "Can't Publish offer. Offer is not saved as now open or comming soon!",
      }));
      ...
      // Publish offers
      const publishOffer = await dispatch(publishOfferAction(loanOfferId));

      if (publishOffer.meta.requestStatus === "fulfilled") {
        // Show modal
      ...
      <OrderbookLayout PageNav={NavMenu}>
        ...
        >
          <div id="loan" className="dropdown-container underline mr-5">
            <Link to={`/client/offers/offer/edit/${id}/${dealType}/`}>
              View offers
            </Link>
          </div>{" "}
        </div>
        ...
        <div id="orderbook-publish-offer">
          <div id="offer-publication">
                      ...
                      >
                        ...
                        {investorCatCount <= 5 ? (
                          ...
                          <Button
                            title="view less"
                            buttonClass="view-more font-bold"
                            handleClick={() => setInvestorCatCount(5)}
                          />
                        )}
                      </div>
                    </div>
            ...
            <div id="save-as-checkboxes" className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2 sm:col-span-1 text-left sm:text-right checkboxes">
                <input
                  type="checkbox"
          ...
          >
            ...
            >
              ...
              <i
                className={`${
                  isLoading && "fa fa-spinner fa-pulse fa-1x fa-fw"
                }`}
              ></i>
            </Button>
          </div>
